---
type: trade-review
status: active
created: 2026-07-03
updated: 2026-07-03
tags: [trade-review, swing-trading, nse, midcapetf, open-position]
sources:
  - ../../analysis/2026-06/2026-06-28-nse-swing-scan-analysis1.md
confidence: medium
trade_date: 2026-07-01
tickers: [MIDCAPETF]
strategy: swing-trade
position_status: open
direction: long
---

# MIDCAPETF Trade Narrative

This page records narrative details for the MIDCAPETF trade opened in July 2026.

## Raw Trade Reference

- **Instrument:** MIDCAPETF, ETF, NSE.
- **Direction:** Long.
- **Entry date:** 2026-07-01.
- **Quantity:** 2000 units.
- **Entry price:** 23.32.
- **Gross entry value:** 46,640.00, excluding brokerage, taxes, slippage, and other charges.
- **Position status:** Open as of 2026-07-03.

## Planned Thesis

The entry follows the 2026-06-28 NSE swing scan, which classified MIDCAPETF as a **Strong Setup** using data as of 2026-06-25. The scan showed Stage 2 markup, 7/8 Trend Template alignment, support near 22.83, and resistance near 23.34.

## Trade Plan

The target, stop loss, and R:R were supplied with the trade record on 2026-07-03:

| Field | Value |
|---|---:|
| Target | 24.4 |
| Stop loss | 22.6 |
| R:R | 1.7:1 |

These values align with the scan's current-price scenario: entry 23.26, stop 22.60, target 24.35, R:R 1.7:1.

## Actual Execution

| Field | Value |
|---|---:|
| Entry date | 2026-07-01 |
| Quantity | 2000 |
| Entry price | 23.32 |
| Gross entry value | 46,640.00 |
| Target | 24.4 |
| Stop loss | 22.6 |
| R:R | 1.7:1 |
| Risk per unit | 0.72 |
| Total planned risk | 1,440.00 |

## Outcome

Position is open. Exit, P&L, and post-trade review will be recorded after a sell is entered in [all-trades-2026](../all-trades-2026.md).

## What Worked

- Entry aligned with a pre-existing Strong Setup.
- Trade plan values were recorded alongside the entry.

## What Failed

- No failure recorded yet; trade remains open.

## Rule Adherence

- **Setup alignment:** High, based on the 2026-06-28 NSE scan.
- **Risk definition:** Medium; stop and target are recorded.
- **Execution status:** Open position; entry and risk plan are recorded, exit review pending until a sell is recorded.

## Lessons

- Review after exit to see whether MIDCAPETF behaved as a lower-volatility Stage 2 ETF setup.

## Updates To Strategy / Setup Pages

- No strategy page changes yet.

## Related

- [2026-06-28 NSE swing scan](../../analysis/2026-06/2026-06-28-nse-swing-scan-analysis1.md)
- [All Trades 2026](../all-trades-2026.md)
- [Watchlist](../../../watchlist.md)
